...
~\\
To compute a CI, we usually need to know the sampling distribution of the estimator $\hat{\theta}$,
...
% Recall: 
% \begin{enumerate}
% \item Draw $B$ random samples of size $n$, with replacement, from the original dataset.
% \item Compute bootstrap estimates $\hat{\theta}_1^*,\ldots,\hat{\theta}_B^*$ using the bootstrap samples.
% \item The collection $\hat{\theta}_1^*,\ldots,\hat{\theta}_B^*$ approximates 
...
% % are reasonable estimates of $\theta$ and 
% % $\sqrt{\text{Var}(\hat{\theta})}=\text{SE}(\hat{\theta})$.
% % ~\\~\\
% % We use these bootstrap estimates in the approximate 95\% CI
% % $$
% % \overline{\theta^*} \pm 2 \times \text{SE}_{\text{boot}}(\hat{\theta})
% % $$
% \end{frame}
...
\item without restrictive assumptions (e.g. normal distribution)
